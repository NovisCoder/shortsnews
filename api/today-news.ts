...
const TODAY_NEWS_API_URL = "https://YOUR-PROJECT.vercel.app/api/today-news";
...
export default function GeneratePage() {
  ...
  const buildFallbackNews = (): TodayNewsResponse => ({
    headline: "오늘의 테스트 핵심 뉴스: 금리·물가·부동산 이슈 정리",
    summary:
      "실제 뉴스 호출이 실패해서 테스트 데이터를 대신 표시하고 있습니다. Vercel 함수 URL과 배포 상태를 확인해 주세요.",
  ...
  const handleFetchTodayNews = async () => {
    try {
      ...
      const response = await fetch(TODAY_NEWS_API_URL, {
        method: "GET",
      });
      ...
      if (!data?.headline || !data?.articles?.length) {
      ...
      setTodayNews(buildFallbackNews());

      toast({
        title: "뉴스 불러오기 실패",
        description: `${message} — 테스트 데이터로 대체했습니다.`,
        variant: "destructive",
      });
    } finally {
    ...
    <div className="max-w-2xl mx-auto animate-in space-y-8">
      ...
      <div className="rounded-xl border border-border bg-card p-6 space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Vercel 서버리스 함수를 통해 실제 뉴스 데이터를 불러옵니다.
          </p>
        </div>
        ...
        <div className="rounded-lg bg-muted/50 p-4 text-sm text-muted-foreground min-h-[100px] space-y-3">
          {!todayNews ? (
            <p>버튼을 누르면 오늘의 실제 뉴스 기사 또는 테스트 데이터를 불러옵니다.</p>
          ) : (
            <>
              <div className="space-y-1">
